...
from chainerrl_autoencoder.experiments_ae.AutoEncoderModels import FCN
import torch
...
autoencoder = FCN().cuda()
autoencoder = autoencoder.double()
autoencoder = torch.nn.DataParallel(autoencoder)
autoencoder.load_state_dict(torch.load('FCNWeight'))
...
def obs2tensor(obs):
	gray_scale_obs = obs2gray(obs)
	final_obs = np.reshape(gray_scale_obs, (1, 1, 600, 400))
	double_final_obs = np.float64(final_obs)
	tensor_obs = torch.from_numpy(double_final_obs)
	return tensor_obs
...
def train_agent(agent, env, steps, outdir, max_episode_len=None,
	...
	obs = env.reset()
	encoded_obs, decoded_obs = autoencoder.forward(obs2tensor(obs))
	...
	try:
		while t < steps:
			...
			encoded_obs, decoded_obs = autoencoder.forward(obs2tensor(obs))
			action = agent.act_and_train(encoded_obs.cpu().data.numpy(), r)
			...
			if done or episode_len == max_episode_len or t == steps:
				# print("Reward for last action: ", r)
				# print("Total reward for episode: ", episode_r)
				print(episode_r)
				encoded_obs, decoded_obs = autoencoder.forward(obs2tensor(obs))
				agent.stop_episode_and_train(encoded_obs.cpu().data.numpy(), r, done=done)
				...
				create_reward_curve(reward_list)
				episode_r = 0
				episode_idx += 1
				episode_len = 0
				obs = env.reset()
